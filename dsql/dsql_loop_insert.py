...
def main(cluster_endpoint):
    ...
    try:
        # Establish connection with connection retry logic
        conn = establish_connection(cluster_endpoint, region, retry_strategy)
        conn.set_autocommit(True)
        cur = conn.cursor()

        # Create table with operation retry logic
        def create_table():
            cur.execute("""
        ...
        for i in range(100):
            def insert_and_verify():
                ...
                cur.execute(
                    "INSERT INTO owner(name, city, telephone) VALUES(%s, %s, %s) RETURNING id",
                    data
                )
                inserted_id = cur.fetchone()[0]

                # Verify insertion
                cur.execute("SELECT * FROM owner WHERE id = %s", (inserted_id,))
                record = cur.fetchone()
